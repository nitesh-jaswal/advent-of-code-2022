...
fn is_pair_any_overlap(assignment_pair: &Vec<&str>) -> bool {
    ...
    let val_1 = second_section.start - first_section.end;
    let val_2 = first_section.start - second_section.end ;
    if val_1 >= 0 && val_2 >= 0 {
        true
    }
    ...
    if let Ok(lines) = lazy_file_reader(input_path) {
        for line in lines {
            if let Ok(assignment_pair) = line {
                ...
                if is_pair_any_overlap(&assignment_pair) {
                    num_full_overlaps += 1;
                }
            }
        }
